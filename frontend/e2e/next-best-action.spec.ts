...
test.describe('next best action', () => {
  ...
  test('ranks real records, filters them, explains and acts on one', async ({ page, tenant }) => {
    ...
    // Level 1: the rules engine's own action for a deal past its close date,
    // with the reason it fired — no model involved.
    const engineActions = page.locator('section', {
      has: page.getByRole('heading', { name: 'Next best actions' }),
    });
    await expect(engineActions.getByText('Mark as a high-risk deal')).toBeVisible();
    await expect(engineActions.getByText(/The expected close date passed \d+ days ago\./)).toBeVisible();
    await page.keyboard.press('Escape');

    // Action type narrows to records with an engine action in that category.
    await page.getByLabel('Filter by action type').selectOption('RISK');
    await expect(deal).toBeVisible();
    await page.getByLabel('Filter by action type').selectOption('');
    ...
    // The rules behind those actions are readable; changing them is admin-only.
    await page.getByRole('tab', { name: 'Rules engine' }).click();
    await expect(page.getByRole('heading', { name: 'Rules engine' })).toBeVisible();
    await expect(page.getByRole('listitem', { name: 'Past its close date → mark high risk' })).toBeVisible();

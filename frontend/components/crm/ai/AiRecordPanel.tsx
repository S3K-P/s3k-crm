...
} from '@/components/crm/ai/nba/NbaRecommendation';
import NbaRecordActions from '@/components/crm/ai/nba/NbaRecordActions';
import { recommendationOf } from '@/components/crm/ai/nba/nba-view';
...
function NextBestActionCard({ entityType, entityId }: { entityType: EntityKind; entityId: string }) {
  ...
  return (
    <section className="surface bd rounded-2xl border p-5">
      ...
      {error && <p className="mt-2 text-[12px] text-rose-600">{error}</p>}
      <div className="mt-3">
        <NbaRecordActions kind={entityType === 'LEAD' ? 'LEAD' : 'OPPORTUNITY'} id={entityId} />
      </div>
      {generation === undefined && <p className="txt-muted mt-2 text-[12.5px]">Loading…</p>}
      <div className="mt-3">
        {recommendation ? (

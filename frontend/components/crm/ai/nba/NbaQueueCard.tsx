...
/* ============================================================
   ...
   One ranked record: who it is, what to do (the engine's
   actions, then the stored AI recommendation or its absence),
   why it ranks here, the CRM facts behind that, and the three
   calls to action.
...
export default function NbaQueueCard({
  ...
  item: PriorityScore;
  rank: number;
  /** Carry out, draft, finish or dismiss the engine's actions. */
  ...
  return (
    ...
    >
      <div className="grid gap-4 lg:grid-cols-[minmax(0,5fr)_minmax(0,7fr)] xl:grid-cols-[minmax(0,4fr)_minmax(0,6fr)_minmax(0,2.6fr)]">
        ...
        {/* Engine actions, AI recommendation & reason */}
        <div className="min-w-0 space-y-2">
          <NbaEngineActions item={item} handlers={engine} />
          {recommendation ? (

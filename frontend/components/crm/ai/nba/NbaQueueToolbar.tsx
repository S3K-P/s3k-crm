...
import type { NbaAction, NbaCategory } from '@/features/ai/ai-insights';
import { cn } from '@/lib/utils';
import { CATEGORY_LABEL, CATEGORY_ORDER } from './nba-helpers';
...
export interface QueueFilters {
  ...
  /** Records with at least one engine action in this category. */
  category: NbaCategory | '';
  /** Records with at least one engine action from this level. */
  actionLevel: NbaAction['level'] | '';
}
...
const ACTION_LEVEL_OPTIONS = [
  { value: '', label: 'Rules and predictive' },
...
export default function NbaQueueToolbar({
  ...
  categoryCounts: Record<NbaCategory, number>;
  availableSignals: SignalFilter[];
  shown: number;
  total: number;
}) {
  const active =
    filters.search.trim() !== '' || filters.level !== '' || filters.signals.length > 0 ||
    filters.scope !== 'ALL' ||
    filters.category !== '' ||
    filters.actionLevel !== '';
          ...
          <FilterSelect
            options={[
              ...
              ...CATEGORY_ORDER.map((category) => ({
                value: category,
                label: `${CATEGORY_LABEL[category]} (${categoryCounts[category]})`,
              })),
            ]}
            value={filters.category}
            onChange={(event) => onChange({ ...filters, category: event.target.value as QueueFilters['category'] })}
            aria-label="Filter by action type"
          />
          <FilterSelect
            options={ACTION_LEVEL_OPTIONS}
            value={filters.actionLevel}
            onChange={(event) =>
              onChange({ ...filters, actionLevel: event.target.value as QueueFilters['actionLevel'] })
            }

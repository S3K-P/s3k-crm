...
export default function NextBestActionPage() {
  ...
  /* The first load is the effect's own, not a call to `load`: the initial
     state is already `loading` with no error, so re-setting both synchronously
     on mount would be a cascading render for no change. Resolution is guarded
     by `cancelled` so an unmount mid-load — or StrictMode's double invoke —
     cannot write to a gone component. `load` remains for the retry button,
     where setting the pending state synchronously is the point. */
  useEffect(() => {
    let cancelled = false;
  ...
  /* Filtering can shrink the result set below the page the reader is on.
     Clamping is derived during render rather than corrected by an effect
     afterwards: an effect would render one frame of an empty table first, and
     `page` itself is left alone so widening the filter again returns the

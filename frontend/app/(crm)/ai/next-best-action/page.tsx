...
import SampleDataNotice from '@/components/crm/ai/SampleDataNotice';
import {
  buildNbaDetail,
...
export default function NextBestActionPage() {
  ...
  /* ---- Load the working set ----
     ...
     matching `dashboards/page.tsx`: `react-hooks/set-state-in-effect` reads
     through a `useCallback` and flags the setState it can reach, and it is
     right to — a load that resolves after the reader has navigated away should
     not touch state, which is what the `cancelled` flag is for. */
  const [reloadCount, setReloadCount] = useState(0);

  // Returning to the loading state is the *retry button's* job, not the
  // effect's: doing it in an event handler is a single render, while doing it
  // in the effect body would set state during a render the effect is reacting
  // to. First load needs no reset — `loading` already starts true.
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const loaded = await getMockNBARecords();
  ...
  // Keep the page in range when filtering shrinks the result set. Clamped
  // during render rather than corrected afterwards in an effect: an effect
  // would first paint an empty page and then re-render with the right one,
  // and `page` is a request for a page, not the page itself.

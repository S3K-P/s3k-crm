...
Infrastructure rather than a CRM concern: Platform modules paginate too, and
``app.platform.*`` may not import ``app.products.*``
(ARCHITECTURE-BOUNDARIES.md rule 1). This module is therefore the single
definition, and :mod:`app.products.crm.shared.pagination` re-exports it so the
CRM import path every list endpoint already uses keeps working unchanged.

...
- [ ] CI green on `main`: lint, types, tests, dependency scan — *pipeline exists as of 2026-08-26 (`.github/workflows/ci.yml`, `P0-W03-DO-01`); this stays unticked until it has actually run green on `main`, which is a fact about a run rather than about the file*
- [ ] FastAPI serves `/health` with structured JSON logs and trace IDs
- [x] A real Postgres (plus Redis and MinIO) runs the suite in CI — via GitHub Actions **service containers** rather than testcontainers (CR17). The property that mattered was a real database, because RLS is enforced by PostgreSQL and a stub would prove nothing about tenant isolation
...
- [x] Boundary linter blocks Platform→CRM imports — ruff's `flake8-tidy-imports.banned-api` rejects `app.products` from the Platform layer, with per-file exemptions for the three composition roots. The rule already existed; `P0-W03-DO-02` was satisfied by making it run on every push rather than by writing a linter
...
- [x] Product access blocks CRM routes without the `s3k-crm` entitlement — implemented 2026-08-26 (`P1-W08-BE-01/02`, R10, CR18). All 35 CRM routes sit behind a single gate; an organization without a usable entitlement gets 403 `product_not_licensed` regardless of the permissions its roles hold. Existing tenants were back-filled by revision `20260826_0200` and new ones are entitled in the same transaction that creates them, so the control shipped without an outage
...
* **Products** — backend complete 2026-08-26 (`P1-W08-BE-01/02`, R10): catalogue,
  entitlements, and the gate covering all 35 CRM routes, with 21 tests. `RLS`
  is ☑ for `product_entitlements`; `platform.products` is the catalogue and
  carries no tenant column by design. **Frontend Wired is ☐** and the row is
  therefore ◐: `GET /products/entitlements` exists so a client can explain a
  403, but nothing consumes it yet, so a tenant whose licence lapses currently
  sees a bare error rather than an explanation. `Mock Removed` is n/a — this
  module never had a mocked screen.
...
| R10 | Missing product-access controls           | P1-W08-BE-02, P1-W08-QA-01                                 | W08         | ☑ (`platform.products` + `platform.product_entitlements`, revision `20260826_0200`; the gate is a router-level dependency covering all 35 CRM routes — CR18. 21 tests, every positive paired with its refusal: absent, suspended, revoked and expired grants are all 403, and holding the whole permission catalogue does not substitute for a licence) |
...
| CR13 | 2026-08-21 | W19 | One `Attachment` table with inline `entity_type`/`entity_id`, instead of doc 09's `Document` + `DocumentVersion` + `DocumentLink` and its separate `POST /documents/{id}/links` step | The single-table shape shipped with `P2-W19-BE-01` and is what exists in the database. Building the richer model now would have meant migrating a table that already carries RLS, for versioning and many-to-many linking that nothing in the CRM asks for yet. Upload-url and link collapse into one call, which also removes a state where a document exists attached to nothing. | Endpoints are `/attachments/*` rather than `/documents/*`. Versioning is deferred; adding it later is a new table plus a foreign key, not a rewrite | *pending approval* — **implementation verified 2026-08-26**: `platform.attachments` is the single table described, RLS-enabled and FORCEd, endpoints served at `/attachments/*`. No `documents`, `document_versions` or `document_links` table exists. Class A by the §15 matrix (no gate or date change); the judgement left is whether deferring versioning is acceptable |
| CR14 | 2026-08-24 | W19 | The dashboard ships as **one** `/crm/dashboard/summary` endpoint rather than the separate summary, pipeline and recent-activity endpoints of `P2-W19-BE-05/06` | Every widget on the screen is rendered from one page load, so three endpoints would have meant three round trips and three chances for the KPI above a list to disagree with the list itself. One response resolves each owner-scoped module's visibility separately (`DashboardScope`), which is what keeps each count equal to the rows behind it. | The *team* filter named in `BE-06` is served by `VIEW_TEAM` (B02) rather than by a query parameter: scope follows the caller's permissions, not a client-supplied argument, so a rep cannot request a wider view than they hold | *pending approval* — **implementation verified 2026-08-26**: one route, `GET /crm/dashboard/summary`, returning KPIs, per-stage pipeline, tasks, meetings and activities in a single response. No `/crm/dashboard/pipeline` or recent-activity route exists. Class A (no gate or date change); record-only |
| CR15 | 2026-08-24 | W07 | `VIEW_TEAM` is granted to **Admin only** by the B02 migration — not to Manager, not to User | Granting it to *User* would widen every rep's reach the moment the migration ran; a change of that consequence must be an administrator's decision, not a side effect of deploying. Manager already holds `VIEW_ALL`, which is strictly wider, so it would be redundant there. Admin is the exception for a **structural** reason rather than a permissions one: `SYSTEM_ROLES` expresses Admin as a wildcard precisely so a newly added permission cannot silently leave administrators without it, and `test_the_admin_role_grants_the_whole_catalogue` pins that invariant. Granting it to Admin widens nothing, since Admin already holds `VIEW_ALL` everywhere. | Team visibility is inert for ordinary users until an administrator grants `<module>.VIEW_TEAM` on the Roles screen. The tables, the predicate and the audit trail are all in place waiting for it | **Accepted 2026-08-26 — keep `VIEW_TEAM` Admin-only.** Team visibility therefore stays inert by default: `VIEW_ALL` short-circuits ahead of it for Admin and Manager, and User holds neither, so no seeded role reaches the team branch. Reaching it is a deliberate act — an administrator granting `<module>.VIEW_TEAM` on the Roles screen, or a future role seeded to hold it. Verified in the database: 16 grants, all on the Admin system template, none elsewhere |
| CR18 | 2026-08-26 | W08 | The product-access check is a **router-level dependency declared once at the composition root**, not the ASGI middleware named in `P1-W08-BE-02` | Same guarantee, better failure mode. FastAPI resolves a parent router's dependencies before any handler beneath it, so the gate covers every `/crm/*` route and a newly mounted CRM module cannot omit it. ASGI middleware would match on the literal prefix `"/crm/"`: move the mount, or add a product under a different path, and it silently stops protecting anything — a gate that fails open when the routing changes. A dependency is attached to the routes themselves and travels with them. It also runs *after* tenant context, which is what makes the check possible at all: the organization has already been resolved and proven to belong to the caller. | None on coverage — 35 CRM routes sit behind it, and `test_product_access.py` parameterises the refusal over one route per module rather than trusting a single endpoint | *pending* |
| CR17 | 2026-08-26 | W03 | CI runs the suite against GitHub Actions **service containers** rather than the testcontainers named in `P0-W03-DO-01` | The property `P0-W03-DO-01` is really asking for is *a real PostgreSQL*, because RLS is enforced by the database and a stub proves nothing about tenant isolation. Service containers deliver exactly that, declared beside the job that needs them and started before it, where testcontainers would add a Python dependency that reaches for a Docker socket from inside the runner. The suite already requires real Redis and real MinIO for the same reason, and both are provided the same way. | None on coverage — the same tests run against the same engines. A future need to spin containers *mid-test* (per-test databases, say) would justify revisiting; nothing asks for that today | *pending approval* — **not yet verifiable**: the workflow declares PostgreSQL 18, Redis and MinIO as service containers, but it has never run, because nothing has been pushed. This CR should be confirmed against the first green run rather than against the file. Class A (no gate or date change) |
| CR16 | 2026-08-26 | W20 | Search vectors are **stored generated columns**, not the triggers named in `P3-W20-BE-01`; `P3-W20-BE-05`'s separate backfill migration is subsumed rather than written | A trigger is a thing you can forget. It must be created per table, re-created if the table is rebuilt, and it silently stops reflecting a column somebody adds to the entity later — leaving a vector that is subtly stale rather than obviously broken, which is the worst failure mode a search index has. A generated column is part of the table definition: PostgreSQL maintains it or the write fails. Adding one also computes it for every existing row during the `ALTER TABLE`, so the backfill has no rows left to find and a separate migration would have been a no-op dressed as work. | None on scope or sequence. Two constraints follow: the expression must be IMMUTABLE, so every `to_tsvector` call names `'english'::regconfig` explicitly rather than relying on `default_text_search_config` — which is the stronger behaviour anyway, since the one-argument form stems according to whoever's session wrote the row; and changing what is indexed is an `ALTER TABLE` rather than a trigger edit plus a backfill, which is the same amount of work stated honestly. The expression is declared in both the model and the migration, and `tests/unit/test_search_index_agreement.py` fails if they drift | *pending approval* — **implementation verified 2026-08-26**: all four `crm.search_vector` columns report `is_generated = ALWAYS` in `information_schema`, and no trigger maintains them. Class A (no gate or date change); record-only |
...
| ---- | ---- | ---- | ------- | ------------------------------ |
| GATE 0 | W03 | ☐ never held | — | `P0-W03-AR-01` unstarted. **CI now exists** (`.github/workflows/ci.yml`, 2026-08-26): `DO-01` and `DO-02` are done, and the real-Postgres and boundary-linter criteria are met. Two criteria remain genuinely unmet — no `orval` client (CR04), no idempotency or rate-limit middleware (`P0-W03-BE-03/04`) — and "CI green on `main`" needs an actual run before it can be ticked |
| GATE 1 | W09 | ☐ never held | — | `P1-W09-AR-01` unstarted. **All eight exit criteria are now technically met**: the eighth — product access on the `s3k-crm` entitlement — was implemented 2026-08-26 (`P1-W08-BE-01/02`, R10, CR18), closing the last unmet one. What remains is procedural: nobody has held the review. The Architect can now hold it against evidence rather than against a gap |
